...
BOOL SEC_LISTCLASS::Create( DWORD dwStyle, DWORD dwExStyle, const RECT& rect, CWnd* pParentWnd, UINT nID, CCreateContext* pContext /*= NULL*/)
{
	...
	bRtn = CreateEx( dwExStyle, NULL, NULL, dwStyle, 
					 ...
					 pParentWnd->GetSafeHwnd(), (HMENU)(SEC_UINT)nID, (LPVOID)pContext );
...
int SEC_LISTCLASS::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
   ...
   if (bOK)
   {
//kim	   m_hcurSplit = AfxGetApp()->LoadCursor(IDC_TOOLBAR_HORZDRAG);
...
void SEC_LISTCLASS::OnDestroy(void) 
{
    ...
    // QA: 31692 - We must clean up control bar destruction
...
//@doc SEC_LISTCLASS
//@mfunc Sets the text font for list items.
//@rdesc void 
//@parm LvPaintContext* | pPC | A pointer to the list view paint context that contains the item information.
...
//@doc SEC_LISTCLASS
//@mfunc Sets the text color for list items.
//@rdesc void 
//@parm LvPaintContext* | pPC | A pointer to the list view paint context that contains the item information.
...
//	Make the background color consistent with the tree control.
...
void SEC_LISTCLASS::DrawHeader(LvPaintContext* pPC)
{
	...
	if (RWIsAppThemed() && RWIsThemeActive())
	{
		...
		hTheme = RWOpenThemeData(hWnd, L"HEADER");
	}
#endif
	...
	if (GetColumnCount() >= 1)
	{
		...
		for (i=0; i<nColCount; i++)
		{
			...
			if(rcHeader.right>rcHeader.left)
			{
				if( hTheme )
				{
					...
					if( i == m_nHotHeaderItem )
						iState = HIS_HOT;
#ifndef __RWUXTHEME_H
					hr = RWDrawThemeBackground( hTheme, pPC->pdc->m_hDC, HP_HEADERITEM, iState, &rcHeader, 0 );
#endif
				}
				else
				{
		...
		{
#ifndef __RWUXTHEME_H
			hr = RWDrawThemeBackground( hTheme, pPC->pdc->m_hDC, HP_HEADERITEMRIGHT, HIS_NORMAL, &rcHeader, 0 );
#endif
		}
		else
		{
...
//		if( hTheme )
//		{
//			hr = RWDrawThemeBackground( hTheme, dc.m_hDC, HP_HEADERITEM, HIS_PRESSED, &rcHeader, 0 );
//		}
//		else
		{
...
//			if( nColHit == m_nHotHeaderItem )
//				iState = HIS_HOT;

//			hr = RWDrawThemeBackground( hTheme, dc.m_hDC, HP_HEADERITEM, iState, &rcHeader, 0 );
//		}
//		else
		{
...
void SEC_LISTCLASS::DrawHeaderImage(CDC* pDC, Column* pCol, CRect& rectText, UINT& nFlags)
{
	...
	if(hHdrIcon)
	{
		...
		::DestroyIcon(hHdrIcon); // Destroy the icon created from ExtractIcon above.
	}
}
...
BOOL SEC_LISTCLASS::Update( int nItem, BOOL bLabelOnly, BOOL bEraseBkgnd, BOOL bUpdateBelow, BOOL bUpdateNow)
{
	if (nItem >= GetTopIndex())
	{
		...
		for (i=GetTopIndex(); i<=nItem; i++)
		{
			...
			if ( pItem )
			{
				//	Take care of the uninitialized cases.
...
void SEC_LISTCLASS::RecalcLayout()
{
	...
	if (pHorzSB || pVertSB)
	{
		...
		{
			///////////////////////////////
			// Get current position of the scroll bar, if nothing change, don't bother 
			// to update it.
			///////////////////////////////
			WINDOWPLACEMENT wndpl;
			wndpl.length = sizeof(WINDOWPLACEMENT);
			::GetWindowPlacement( pHorzSB->m_hWnd, &wndpl );

			if ( wndpl.rcNormalPosition.left != rectInside.left ||
				 wndpl.rcNormalPosition.top != rectInside.bottom -1 ||
				 (wndpl.rcNormalPosition.right - wndpl.rcNormalPosition.left) != (rectInside.Width() + 1) || 
				 (wndpl.rcNormalPosition.bottom - wndpl.rcNormalPosition.top) != cyHorzScroll )
			{
				::DeferWindowPos(hDWP, pHorzSB->m_hWnd, NULL, rectInside.left,
	//				rectInside.bottom, rectInside.Width()+1, cyHorzScroll, 0);	// Alignment problem.
					rectInside.bottom-1, rectInside.Width()+1, cyHorzScroll, 0);
				pHorzSB->Invalidate();
			}
		}
		...
		if (pVertSB)
		{
			...
			WINDOWPLACEMENT wndpl;
			wndpl.length = sizeof(WINDOWPLACEMENT);
			::GetWindowPlacement( pVertSB->m_hWnd, &wndpl );

			if ( wndpl.rcNormalPosition.left != rectInside.right ||
				 wndpl.rcNormalPosition.top != rectClient.top ||
				 (wndpl.rcNormalPosition.right - wndpl.rcNormalPosition.left) != cxVertScroll || 
...
//  Fix the incorrect text wrap.
		rcText.right = pPC->pCol->cx-(SEC_LEFT_OFFSET+SEC_RIGHT_OFFSET)-(rcIcon.right-rcIcon.left);
...
//	CRect rcText(0,0,pPC->pCol->cx,0);
	CRect rcText(0,0,rcItem.right-rcItem.left,0);
...
void SEC_LISTCLASS::ClearMeasurements()
{
	...
	for (i=0; i<c; i++)
	{
		...
		{
			pItem->m_bDirty = TRUE;
//  This causes a display problem when the font is changed without word-wrap.
...
int SEC_LISTCLASS::GetItemAbovePoint( const CPoint& pt )
{
	int y = 0;
	int i;
	for(i = 0; i < GetItemCount() && pt.y > y; i++ )
	{
...
BOOL SEC_LISTCLASS::InsertColumn(int nCol, const CString& strHeader, int fmt, int widthCol, int /*iSubItem*/, int iImage, BOOL bUpdate)
{
	...
	pCol->fmt = fmt;
//  Make iSubItem consistent with nCol, otherwise there are weird assertion errors.
...
//
{
	// You can't delete the first column unless it's the only column
	...
	// No column left, remove all items.
	if ( GetColumnCount() == 0 )
		DeleteAllItems();
...
BOOL SEC_LISTCLASS::DeleteAllColumns()
{
	...
	// Call DeleteAllItems to delete all the sub items as well as all the items
	SEC_LISTCLASS::DeleteAllItems();
...
void SEC_LISTCLASS::EnableHeaderCtrl(BOOL bEnable, BOOL bSortHeader)
{
	...
	ModifyListCtrlStyle( (bEnable*LVS_NOCOLUMNHEADER)|(bSortHeader*LVS_NOSORTHEADER),
		(!bEnable*LVS_NOCOLUMNHEADER)|(!bSortHeader*LVS_NOSORTHEADER) );	
}
...
BOOL SEC_LISTCLASS::SetItemData(int nItem, SEC_DWORD dwData)
{
	LV_ITEM lvi;
...
SEC_DWORD SEC_LISTCLASS::GetItemData(int nItem) const
{
	LV_ITEM lvi;
...
void SEC_LISTCLASS::ResizeColumnsToFit()
{
	...
	if( iColumnCount != 0 )
	{
		...
		if ( pCol )
		{
			...
			// Check to see if the width of the subitem columns is greater than the control width
			// and make adjustments if so
			if(rcInside.Width() > 20 + iColumnCount-1 && pCol->cx < 20){
				pCol->cx = 20;
				int subWidth, remWidth; 
				subWidth = remWidth = rcInside.Width() - 20;

				// Divide the remaining width among the other columns.
				for(nCol = 1; nCol < iColumnCount-1; nCol++){
					pCol = GetColumnAt(nCol);
					pCol->cx = (int)(((double)pCol->cx/(double)cxSubItems)*subWidth);
					remWidth -= pCol->cx;
				}
				pCol = GetColumnAt(iColumnCount-1);
				pCol->cx=remWidth;
			}
		}
	}
...
void SEC_LISTCLASS::OnTimer(SEC_UINT nIDEvent) 
{
...
SEC_LISTCLASS::EditLabel_OnUpdate(void)
{
	...
	// To dynamically enlarge the in-place edit control while typing characters.
	Item* pItem = GetItemAt(m_nEditItem);
	if (pItem)
	pItem->m_bDirty = TRUE; 
...
BOOL SEC_LISTCLASS::CreateScrollBarCtrl(DWORD dwStyle, UINT nID)
{
	...
	return (::CreateWindow(_T("SCROLLBAR"), NULL,
		...
		0, 0, 1, 1, m_hWnd, (HMENU)(SEC_UINT)nID,
...
void SEC_LISTCLASS::OnVScroll(UINT nSBCode, UINT /*nPos*/, CScrollBar* pScrollBar)
{
	...
	case SB_THUMBTRACK:
	case SB_THUMBPOSITION:
		ScrollVert(sbi.nTrackPos - sbi.nPos);
		break;
	default:
		break;
	}
...
int SEC_LISTCLASS::HitTest(CPoint pt, UINT* pFlags, int& nHitCol) const
{
	...
	*pFlags = 0;
	...
	else	// No column in the list, return FALSE.
	{
		*pFlags = LVHT_NOWHERE;
		return -1;
	}
...
#ifdef _UNICODE
	if( !(GetListCtrlStyle() & LVS_NOSORTHEADER) )
	{
		if( RWIsAppThemed() && RWIsThemeActive() )
		{
			if( iHitFlags & LVHT_COLUMNSEP ||
				iHitFlags & LVHT_COLUMNHEADING )
			{
				if( m_nHotHeaderItem != nHitCol )
				{
					if( m_nHotHeaderItem >= 0 )
					{
						int tmp = m_nHotHeaderItem;
						m_nHotHeaderItem = -1;
						DrawBtnHeader( tmp, FALSE );
					}

					if( nHitCol >= 0 )
					{
						m_nHotHeaderItem = nHitCol;
						DrawBtnHeader( nHitCol, FALSE );
...
void SEC_LISTCLASS::OnRButtonDown(UINT nFlags, CPoint point)
{
    ...
    if(m_pToolTipWnd->GetSafeHwnd() && m_pToolTipWnd->IsWindowVisible())
    {
        ...
        if(pWndTop) SendMessage(WM_MOUSEACTIVATE, WPARAM(pWndTop->m_hWnd), MAKELPARAM(HTCLIENT, WM_RBUTTONDOWN));
    }	
...
BOOL SEC_LISTCLASS::NotifyEndLabelEdit(int nEditItem, int iSubItem, CString& strEdit, BOOL bCommit) const
{
	...
	if (bCommit)
	{
		...
		//	Set the buffer size too.
		lvdi.item.cchTextMax = LVXC_INTERNALBUFFERSIZE;
	...
	) 
{
	  ...
	  )
	{
		...
		// QA: 32302 - Readding this logic to fix multiple item drag and drop.
		// Selection focus does not appear to be a problem as of CD52.
		if(IsSelected(nHitIndex) && (GetSelectedCount()>1) )
			return TRUE;
...
void SEC_LISTCLASS::OnSysColorChange( )
{
	if ( m_bTrackSystemColors )
	{
		...
		// statics
		rgbNoFocusBk =		::GetSysColor(COLOR_3DLIGHT);
...
LRESULT SEC_LISTCLASS::OnThemeChanged(WPARAM wParam, LPARAM lParam)
{
...
BOOL SEC_LISTCLASS::OnWndMsg(UINT message, WPARAM wParam, LPARAM lParam, LRESULT* pResult)
{
...
// QA: 32259
	...
	return SEC_LISTBASE::OnWndMsg( message, wParam, lParam, pResult );
}
